...
class imaging_layer(nn.Module):
    ...
    def forward(self, u):
        ...
        for i in range(u.size(0)):
            values_temp = (values[i, :]) / (norm_nonzero(values[i, :]))  # Add epsilon to avoid division by zero
...
torch.save(model.state_dict(), "weights_large_relu.pt")