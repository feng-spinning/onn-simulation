...
M = 250
w = 0.051
z = nn.Parameter(torch.tensor([100.0], device=device, dtype=torch.float64))
...
class propagation_layer(nn.Module):
    def __init__(self, L, lmbda):
        super(propagation_layer, self).__init__()
        self.L = L
        self.lmbda = lmbda
        self.z = nn.Parameter(torch.tensor([100.0], device=device, dtype=torch.float64))
...
model = OpticalNetwork(M, L, lmbda).to(device)
loss_function = nn.MSELoss()
# z_params = [layer.z for layer in model.optical_layers if isinstance(layer, propagation_layer)]
# phase_params = [layer.phase_values for layer in model.optical_layers if isinstance(layer, modulation_layer)]
# # magnitude_params = [layer.amplitude_values for layer in model.optical_layers if isinstance(layer, modulation_layer)]
# param_groups = [
#     {'params': z_params, 'lr': 20},
#     {'params': phase_params, 'lr': 0.003}
# ]
z_values = [layer.z for layer in model.optical_layers if isinstance(layer, propagation_layer)]
z_params = [layer.z for layer in model.optical_layers if isinstance(layer, propagation_layer)]
phase_params = [param for layer in model.optical_layers if isinstance(layer, modulation_layer) for param in layer.parameters()]
optimizer = torch.optim.Adam([
    {'params': z_params, 'lr': 1},
    {'params': phase_params, 'lr': learning_rate}
], lr=learning_rate)

# Initialize lists to store z values after each epoch
z_values = [[] for _ in z_values]
...
for epoch in range(epochs):
    ...
    for layer in model.optical_layers:
        if isinstance(layer, propagation_layer):
            z_values[propagation_idx].append(layer.z.item())
            propagation_idx += 1    
...
for idx, z_vals in enumerate(z_values):
    plt.plot(z_vals, label=f"Propagation Layer {idx+1}")
plt.xlabel("Epoch")
plt.ylabel("z value")
plt.title("Evolution of z values during training")
plt.legend()
plt.grid(True)
