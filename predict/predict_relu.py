...
class imaging_layer(nn.Module):
    ...
    def forward(self, u):
        ...
        for i in range(u.size(0)):
            values_temp = values[i, :] / norm_nonzero(values[i, :])
...
model.load_state_dict(torch.load("weights_large_relu.pt",map_location=torch.device(device)))
model.eval()
